document.addEventListener('DOMContentLoaded', () => {
    ...
    // FORCE RESET: UI Update (v9)
    const resetVersion = "v9";
    ...
    const translations = {
        ...
        es: {
            ...
            categories: {
                kpop: { name: "K-Pop y Ent", title: "K-Pop y Entretenimiento", desc: "Lo último del mundo del K-Pop and el entretenimiento coreano." },
        ...
        const newsPosts = [
            {
                ...
                comments: [
                    ...
                    { id: 102, text: "Retro concept is exactly what we needed!", author: "RetroLover", date: "2026-03-12", lang: "en" }
            ...
            },
            {
                ...
                contents: { ko: "샤이니 태민이 지드래곤의 소속사인 갤럭시코퍼레이션과 전속계약을 체결했습니다. 솔로 아티스트로서의 새로운 막을 열게 되었습니다.", en: "SHINee's Taemin has officially signed with Galaxy Corporation, the same agency as G-Dragon, starting a new chapter as a solo artist.", ja: "SHINeeのテミンが、G-DRAGONの所属事務所であるギャラクシーコーポレーションと専属契約を締結しました。", zh: "SHINee 泰民已正式与权志龙所属的 Galaxy Corporation 签署专属合约。", es: "Taemin de SHINee ha firmado oficialmente con Galaxy Corporation, la misma agencia que G-Dragon." },
                comments: [
            ...
            },
            {
                ...
                contents: { ko: "DAY6의 원필이 이번 달 솔로 앨범으로 돌아옵니다. 5월에는 팬들과 가까이 소통할 수 있는 단독 콘서트도 개최할 예정입니다.", en: "Wonpil of DAY6 is returning with a solo album this month and will hold a standalone concert in May.", ja: "DAY6のウォンピルが今月, ソロアルバムで帰ってきます。5월에는 단독 콘서트도 개최할 예정입니다.", zh: "DAY6 的元弼将于本月带着个人专辑回归, 并计划于 5 月举办个人演唱会。", es: "Wonpil de DAY6 regresa con un álbum solista este mes and tendrá un concierto en solitario en mayo." },
                comments: [
                    { id: 301, text: "콘서트 무조건 갑니다!", author: "마이데이", date: "2026-03-12", lang: "ko" },
                    { id: 302, text: "Finally a solo concert! So proud.", author: "Day6Fan", date: "2026-03-12", lang: "en" }
                ], views: 1200, likes: 310
            }
        ];
        return newsPosts;
    }
    ...
    function renderPosts() {
        ...
        pagedPosts.forEach(post => {
            ...
            const filteredComments = postComments.filter(comment => !comment.lang || comment.lang === currentLang);
            const commentCount = filteredComments.length;

            let commentsHtml = '';
            filteredComments.forEach(comment => { 
                const isCommentAuthor = comment.author === currentUser; 
                commentsHtml += `<div class="comment-item"><div class="comment-header"><span>@${comment.author} • ${comment.date}</span><div class="comment-actions">${isCommentAuthor ? `<button class="btn-icon delete-comment" data-post-id="${post.id}" data-comment-id="${comment.id}">🗑</button>` : ''}</div></div><div class="comment-content">${comment.text}</div></div>`; 
            });
        ...
        postsContainer.querySelectorAll('.share-post').forEach(btn => btn.addEventListener('click', (e) => { e.stopPropagation(); sharePost(parseInt(e.target.dataset.id)); }));
    ...
    function sharePost(postId) { const url = `${window.location.origin}${window.location.pathname}?post=${postId}`; navigator.clipboard.writeText(url).then(() => { alert(currentLang === 'ko' ? "링크가 복사되었습니다!" : "Link copied to clipboard!"); }).catch(err => { console.error('Failed to copy link: ', err); }); }
